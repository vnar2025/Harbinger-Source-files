...
class GraphicView extends ComponentView {
  ...
  events() {
    return {
      'click .js-graphicSlider-link': 'onClick',
      'keydown .js-graphicSlider-scrollbar': 'onKeyDown'
    };
  }

  postRender() {
    this.$('.graphicSlider__widget').imageready(() => {
      this.setReadyStatus();
      this.setupInviewCompletion('.graphicSlider__widget');
  ...
  setupScrollable() {
    ...
    const $scrollbar = this.$('.js-graphicSlider-scrollbar');
  ...
  onClick(event) {
    ...
    const item = this.model.get('_graphicSlider');
...
GraphicView.template = 'graphicSlider.jsx';

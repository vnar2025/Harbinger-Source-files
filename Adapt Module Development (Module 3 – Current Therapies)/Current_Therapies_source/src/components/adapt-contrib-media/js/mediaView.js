...
class MediaView extends ComponentView {
  ...
  setupPlayer() {
    ...
    // Set autoplay and muted attributes directly on the media element
    const $mediaEl = this.$('audio, video');
    if (modelOptions.autoplay) {
      $mediaEl.attr('autoplay', true);
    }
    if (modelOptions.muted) {
      $mediaEl.attr('muted', true);
    }
  ...
  onPlayerReady(mediaElement, domObject) {
    ...
    const autoplay = this.model.get('_playerOptions')?.autoplay;
    if (autoplay) {
      const container = this.$('.mejs-container')[0];
      const observer = new IntersectionObserver(
        (entries) => {
          entries.forEach(entry => {
            if (entry.isIntersecting) {
              player.play().catch(() => {}); // play only when in view
            } else {
